...
public class Worm : Bug {
    ...
    public new void OnCollisionEnter2D(Collision2D c) {
        ...
        if (c.gameObject.tag == "worm" && !c.gameObject.GetComponent<WormPart>().isHead && c.gameObject.GetComponent<WormPart>().state == WormPartStates.Detatched) {
            int insertIndex = 1;
            try {
                insertIndex = bodyParts.IndexOf(bodyParts.Last(w => w.state == WormPartStates.Alive)) + 1;
            }
            catch (System.InvalidOperationException) {
                head.GetComponent<WormPart>().Kill(WormPartStates.Alive);
            }
